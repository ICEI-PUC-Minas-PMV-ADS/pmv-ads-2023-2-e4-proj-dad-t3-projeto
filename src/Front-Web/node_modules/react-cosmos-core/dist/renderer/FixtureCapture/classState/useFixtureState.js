...
import { createFixtureStateClassState, findFixtureStateClassState, getFixtureStateClassState, removeFixtureStateClassState, } from '../../../fixtureState/classState.js';
...
export function useFixtureState(fixture, decoratorId, elRefs) {
    ...
    useEffect(() => {
        ...
        elPaths.forEach(elPath => {
            ...
            if (!fsClassState) {
                ...
                const prevFsClassState = findFixtureStateClassState(prevFixtureState.current, elementId);
                if (prevFsClassState && !isEqual(prevFsClassState, fsClassState)) {
                    return replaceState(elRef, extendWithValues(elRef.state, fsClassState.values));
                }
            }
        });
    }, [
    ...
    function handleRef(elPath, elRef) {
        ...
        const elementId = { decoratorId, elPath };
        const fsClassState = findFixtureStateClassState(lastFixtureState.current, elementId);
        if (!fsClassState) {
            setFixtureState(prevFs => ({
                ...prevFs,
                classState: createFixtureStateClassState({
